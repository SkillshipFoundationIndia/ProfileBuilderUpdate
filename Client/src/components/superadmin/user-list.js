...
export default class UserList2 extends Component {
  ...
  getAdmin1() {
    AuthService.getUsers(this.props.match.params.id)
      .then(response => {
        ...
        console.log(this.state.list.username);
      })
      .catch(e => {
        console.log(e);
      });
  ...
  retrieveUsers() {
    ...
    AuthService.getAdmin1(this.props.match.params.id)
      .then(response => {
        this.setState({
          users: response.data
        });
  ...
  render() {
    const { searchTitle, users,list, currentUser,currentAdmin, currentIndex } = this.state;
    ...
    return (
      ...
      <h4>City Heads under <strong><u>{this.state.list.username}</u></strong></h4>
          <Button
           className="btn btn-sm"
           color="btn btn-elegant accent-3"
           type="submit"
