...
import TasksList2 from "./task-list2"
import UserList2 from "./user-list2"
import Task from "./task.component2"
...
export default class BoardModerator2 extends Component {
  constructor(props) {
    super(props);
  ...
  componentDidMount() {
    ...
    this.setState({ currentUser: currentUser,id:currentUser.id, userReady: true })
    ...
    UserService.getAdminBoard2().then(
      response => {
        this.setState({
        ...
        <Tab.Container onClick={() => this.setState({ menuVisible: !this.state.menuVisible })} defaultActiveKey="home" className="bg-dark sidebar">
            ...
            <Col md={4} className="d-none d-lg-block">
                <Nav style={{ backgroundColor: "gray"}} variant="pills pills-white" className="article__title type1 flex-column mt-5 bg-dark sidebar">
                  <Nav.Item >
                    <Nav.Link eventKey="home"   style={{color:"white"}}><span className="type2">Task List</span></Nav.Link>
                  </Nav.Item>
                  <Nav.Item>
              ...
              {loading ?(<Loading/>):(
                ...
                <Tab.Content>
                  ...
                  <TasksList2 className="mt-5"/>   </Tab.Pane>
